...
class CodeView(Static):
  ...
  def show_file(
    ...
    *,
    ...
    header = Static(f"[bold]{path}[/bold]\n")
    ...
    if is_binary:
      self.mount(
        Static(
          "[dim]Binary file — open in browser with [bold]b[/bold][/dim]"
        )
      )
      return
    ...
    highlighted = self._highlight(path, content, language)
    body = Static(highlighted)
